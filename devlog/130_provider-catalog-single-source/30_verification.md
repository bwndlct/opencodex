...
| Key-login projection | `KEY_LOGIN_PROVIDERS` equals `deriveKeyLoginMap()`. |
...
| Risk | Status |
|------|--------|
| GUI endpoint unavailable | The modal keeps a minimal `custom` fallback, matching the previous silent-failure behavior while avoiding stale hardcoded provider rows. |
