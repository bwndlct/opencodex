...
import { overridesDependingOnCombo, overridesDependingOnProvider } from "../model-route-overrides";
...
export async function handleManagementAPI(req: Request, url: URL, config: OcxConfig, deps: ManagementApiDeps = {}): Promise<Response | null> {
  ...
  // Fork-specific endpoints (routing-settings, incidents, health, sessions, model-route-overrides, drain)
  // are dispatched to the consolidated fork-api-handlers module.
  const forkResponse = await handleForkEndpoints(req, url, config, {
    readUsageEntries: deps.readUsageEntries,
    healthNow: deps.healthNow,
    saveConfig: deps.saveConfig,
    refreshCodexCatalog: deps.refreshCodexCatalog,
  });
