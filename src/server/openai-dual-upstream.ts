...
import { readConfigDiagnostics, saveConfig } from "../config";
...
export function effectiveOpenAiRoutePolicy(
  ...
  return config.openAiDualUpstream?.defaultPolicy ?? "company_first";
}
...
export async function runOpenAiDualUpstream(
    ...
    try {
      ...
      if (
        ...
        && (
          ...
          || (persistedDual.defaultPolicy ?? "company_first") !== (dual.defaultPolicy ?? "company_first")
          || (persistedDual.autoSwitchToCompany ?? true) !== (dual.autoSwitchToCompany ?? true)
        )
      ) {
  ...
  for (let upstreamIndex = 0; upstreamIndex < order.length; upstreamIndex += 1) {
    ...
    if (policy === "personal_first") {
      ...
      if (sessionPolicy === "inherit" && dual.autoSwitchToCompany !== false) {
